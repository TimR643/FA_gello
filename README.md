...
If you stop `launch_robot.py` with `Ctrl+C`, Polymetis may print a `CalledProcessError ... SIGINT/SIGKILL` traceback during shutdown.
This is usually a shutdown artifact (not a camera error). Restart with `./start_gello_panda.sh` and continue.
