...
def main(args):
    if args.mock:
        ...
        if args.wrist_camera_url is not None:
            source = args.wrist_camera_url.strip().rstrip("#")
            if "<rtsp_or_gige_url>" in source:
                raise RuntimeError("Please replace <rtsp_or_gige_url> with your real camera stream URL.")
            print(f"Using wrist network camera URL: {source}")
            camera_clients["wrist"] = OpenCVCamera(source=source)
