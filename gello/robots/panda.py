...
GRIPPER_SPEED = 255
GRIPPER_OPEN_FORCE = 80
GRIPPER_CLOSE_FORCE = 120
GRIPPER_MAX_STEP = 0.015
GRIPPER_MIN_CLOSE_WIDTH = 0.004
GRIPPER_INIT_SPEED = 180
GRIPPER_INIT_FORCE = 120
GRIPPER_INIT_SETTLE_S = 0.15
GRIPPER_INIT_CLOSE_RATIO = 0.35
GRIPPER_PRESS_DELTA = 0.25
GRIPPER_RELEASE_DELTA = 0.10
...
class PandaRobot(Robot):
    ...
    def __init__(
        ...
        if run_gripper_startup_cycle:
            ...
            self.gripper.goto(width=MAX_OPEN, speed=GRIPPER_SPEED, force=GRIPPER_OPEN_FORCE)

        self._last_gripper_width = float(np.clip(self.gripper.get_state().width, 0.0, MAX_OPEN))
        self._gripper_synced = False
        self._leader_gripper_rest_cmd = 0.0
    ...
    def command_joint_state(self, joint_state: np.ndarray) -> None:
        ...
        if not self._gripper_synced:
            self._last_gripper_width = float(np.clip(self.gripper.get_state().width, 0.0, MAX_OPEN))
            self._leader_gripper_rest_cmd = gripper_cmd
            self._press_latched = False
            self._gripper_synced = True
            return

        # Toggle gesture (button-like):
        # move the leader gripper away from the relaxed startup position -> toggle
        # release back near relaxed position -> arm next toggle
        press_metric = abs(gripper_cmd - self._leader_gripper_rest_cmd)
        if press_metric >= GRIPPER_PRESS_DELTA and not self._press_latched:
            self._gripper_toggle_closed = not self._gripper_toggle_closed
            self._press_latched = True
        elif press_metric <= GRIPPER_RELEASE_DELTA:
            self._press_latched = False

        target_width = GRIPPER_MIN_CLOSE_WIDTH if self._gripper_toggle_closed else MAX_OPEN
        target_force = GRIPPER_CLOSE_FORCE if self._gripper_toggle_closed else GRIPPER_OPEN_FORCE

        # Open commands should release reliably without extra delay. For closing,
        # keep rate limiting to avoid sudden jerks.
        if self._gripper_toggle_closed:
            ...
            limited_target_width = MAX_OPEN

        if abs(limited_target_width - self._last_gripper_width) > GRIPPER_CMD_EPS:
            self.gripper.goto(
                width=limited_target_width, speed=GRIPPER_SPEED, force=target_force
            )
