"""Incremental HDF5 logging for LeRobot teleoperation and policy runs."""
...
class H5RobotLogger:
    ...
    The logger deliberately lives in the robot plugin rather than in a rollout
    launcher.  Consequently every caller of ``get_observation``/``send_action``
    is supported, including the official ``lerobot-record`` teleoperation loop.
    """
    ...
    def __init__(self, path: str | Path, *, num_dofs: int, flush_every: int = 1):
        ...
        action["sent"].attrs["description"] = "command sent after safety limiting"
        self._h5.flush()
    ...
    def log_observation(
        ...
        # Reserve the matching action row. It is filled by send_action below.
