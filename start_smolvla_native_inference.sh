...
: "${CKPT:=$HOME/lerobot_outputs/train/bottle_task_smolvlabase/checkpoints/060000/pretrained_model}"
...
: "${TASK:= Lift the bottle and put it to the right-hand side space}"
: "${DURATION:=200}"
: "${NUM_EPISODES:=1}"
: "${FPS:=10}" #for large models 8.3 Hz seems to be the maximum FPS for execution, models are however working fine 
...
#sometimes abvoce solution does not work, the use follwoing two lines as command 

#POLICY_CAMERA_NAMES_RESOLVED="${POLICY_CAMERA_NAMES:-wrist,base}"
#LIVE_CAMERA_NAMES_RESOLVED="${LIVE_CAMERA_NAMES:-wrist,base}"
