...
: "${CKPT:=$HOME/lerobot_outputs/train/push_white_cube_50-50_smolvlabase/checkpoints/060000/pretrained_model}"
# Push the white cube straight forward.
# Push the white cube forward, then guide it right through the opening.
: "${TASK:=Push the white cube forward, then guide it right through the opening.}"
: "${DURATION:=150}"
: "${FPS:=10}"
